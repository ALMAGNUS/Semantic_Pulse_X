#!/usr/bin/env python3
"""
Scraper Yahoo Actualités FR (simple, indépendant, low-code)

Usage exemples:
  - python scripts/scrape_yahoo.py --url https://fr.news.yahoo.com/... --pays FR --domaine politique
  - python scripts/scrape_yahoo.py --urls urls.txt --pays FR --domaine politique
  - python scripts/scrape_yahoo.py --discover 3 --pays FR --domaine politique   # auto-découverte depuis la page d'accueil

Sortie:
  - JSON normalisé dans data/raw/scraped/yahoo_YYYYmmdd_HHMMSS.json
...
def fix_mojibake(text: str) -> str:
    """Corrige les accents mal décodés typiques (Ã, Â, etc.)."""
    if not text:
        return text
    if "Ã" in text or "Â" in text:
        try:
            # Tentative de correction latin-1 -> utf-8
...
def fetch_html(url: str, timeout: int = 15) -> str:
    ...
    # Utiliser le contenu brut + parser gère mieux l'encodage
    # Conserver fallback apparent_encoding si nécessaire ailleurs
    return resp.content.decode(resp.apparent_encoding or resp.encoding or "utf-8", errors="replace")


def parse_yahoo_article(html: str, url: str) -> dict[str, Any]:
    # Parser lxml pour une meilleure robustesse
    soup = BeautifulSoup(html, "lxml")

    # Sélecteurs simples et robustes (fallbacks)
    title = None
    ...
    # Résumé (chapeau)
    resume = None
    for sel in ["h2", "header h2", "article h2", "meta[name='description']"]:
        node = soup.select_one(sel)
        if node:
            if node.name == "meta":
    ...
    paragraphs: list[str] = []
    for sel in ["article p", "div p", "section p"]:
    ...
    # Auteur
    auteur = None
    for sel in ["[itemprop='author']", "span.author", "a.author", "meta[name='author']"]:
        node = soup.select_one(sel)
        if node:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scraper Yahoo Actualités FR")
    ...
    parser.add_argument("--discover", type=int, default=0, help="Nombre d'articles à auto-découvrir depuis la page d'accueil Yahoo FR")
    ...
    if not urls and args.discover > 0:
        try:
            home_html = fetch_html("https://fr.news.yahoo.com/")
            ...
            for a in soup.select("a"):
                ...
                if not href:
                    continue
                if href.startswith("/"):
                    href = "https://fr.news.yahoo.com" + href
                if href.startswith("https://fr.news.yahoo.com/") and href not in found:
                    # filtrage grossier: éviter les sections génériques
                    if any(seg.isdigit() for seg in href.split("-")) or "html" in href:
                        found.append(href)
                if len(found) >= args.discover:
                    break
            urls = found
            if not urls:
                print("⚠️ Auto-découverte n'a trouvé aucun article sur la page d'accueil.")
    ...
    for u in urls:
        try:
            html = fetch_html(u)
            item = parse_yahoo_article(html, u)
