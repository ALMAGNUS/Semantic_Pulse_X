...
def show_workflow_visualization():
    ...
    for i, (step, status) in enumerate(zip(workflow_steps, step_statuses, strict=False)):
    ...
    steps_data = []
    for step, status in zip(workflow_steps, step_statuses, strict=False):
        steps_data.append({
            "Étape": step,
            "Statut": status,
